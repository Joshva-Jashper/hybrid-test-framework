...
`test_login.py`
|
 Valid login, invalid credentials, logout 
|
|
`test_register.py`
|
...
`test_cart.py`
|
 Add to cart, quantity update, checkout, payment 
|
|
`test_product.py`
|
 Product search, filter by category/brand 
|
|
`test_contact_us.py`
|
...
`test_user.py`
|
 Create user, duplicate email, delete user, get by email, wrong data, missing fields 
|
|
`test_login_api.py`
|
 Successful login, invalid credentials 
|
|
`test_products_api.py`
|
 Get all products, search by name, category filter, schema validation 
|
|
`test_brand.py`
|
...
|
|
`test_register_api_ui`
|
 Create user via API → verify login via UI 
|
|
`test_register_ui_api`
|
 Register via UI → verify user exists via API 
|
|
`test_delete_user_via_ui`
|
 Create + delete user via API → verify login failure via UI 
|
...
`retain-on-failure`
|
 When to save Playwright traces 
|
|
`--alluredir`
|
...
`2`
|
 Auto-retry count for flaky tests 
|
|
`--reruns-delay`
|
...
2.
 Python Setup 
|
 Configure Python 3.11 with pip cache 
|
|
3.
...
4.
 Install Playwright 
|
 Install Chromium + OS system deps 
|
|
5.
 Run Tests 
...
6.
 Upload Artifacts 
|
 On failure: screenshots, traces & Allure results (retained 7 days) 
|
...
**
|
 Validates that each feature works as expected against defined requirements 
|
 All UI & API test cases 
|
|
**
Smoke Testing
**
|
 A quick sanity check on the most critical paths to catch showstopper bugs fast 
|
`@pytest.mark.smoke`
 tests 
|
...
**
|
 Directly tests REST endpoints for correctness, status codes, response schemas, and edge cases 
|
`tests/test_api/`
|
|
...
**
|
 Automates real user interactions in a browser (clicks, form fills, navigation) 
|
`tests/test_ui/`
|
|
...
**
|
 Tests complete user journeys that span both API and UI layers in a single flow 
|
`test_end_to_end.py`
|
|
...
**
|
 Combines API calls for test setup/teardown with UI assertions (and vice versa) for realistic, efficient test flows 
|
`@pytest.mark.e2e`
|
|
...
**
|
 Verifies the system handles invalid inputs, duplicate data, and missing fields gracefully 
|
`test_user.py`
, 
`test_login_api.py`
...
**
|
 Ensures API responses conform to the expected JSON structure 
|
`test_products_api.py`
, 
`test_brand.py`
...
**
|
 Runs the same test logic against multiple data sets from JSON files using 
`pytest.mark.parametrize`
|
 Login, product, category, cart tests 
|
|
**
Regression Testing
**
|
 The full test suite acts as a regression safety net, automatically triggered on every push via CI 
|
