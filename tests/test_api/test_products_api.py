...
@pytest.mark.api
def test_get_product_with_search_parameter(api_client):
    product_client = ProductClient(api_client)
    response = product_client.search_product(data={"search_product": "Blue Top"})
    response_data = response.json()
    expect(response).to_be_ok()
    assert response.status == 200
    assert response.status_text == "OK"
    ...
    assert first_product["name"] == "Blue Top"
    assert first_product["price"] == "Rs. 500"
    assert first_product["brand"] == "Polo"
    assert len(product)>0    

@pytest.mark.api
def test_get_product_without_search_parameter(api_client):
    product_client = ProductClient(api_client)
    response = product_client.search_product(data=None)
    assert response.status == 200
    response_data = response.json()
